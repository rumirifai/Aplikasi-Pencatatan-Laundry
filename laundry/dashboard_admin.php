...
$queryPermintaanCucian = "SELECT pc.no_cucian, pc.tgl_masuk, sc.jenis_status_cucian
                         FROM PermintaanCucian pc
                         INNER JOIN StatusCucian sc ON pc.id_status_cucian = sc.id_status_cucian
                         ORDER BY pc.tgl_masuk DESC LIMIT 5";
$resultPermintaanCucian = mysqli_query($conn, $queryPermintaanCucian);

// Mengambil data Transaksi Terkini
$queryTransaksi = "SELECT t.no_transaksi, st.jenis_status_transaksi, t.subtotal
                   FROM Transaksi t
                   INNER JOIN StatusTransaksi st ON t.id_status_transaksi = st.id_status_transaksi
                   ORDER BY t.no_transaksi DESC LIMIT 5";
        ...
        <table>
          ...
          <tbody>
            ...
            while ($row = mysqli_fetch_assoc($resultPermintaanCucian)) {
              ...
              echo "<td>" . $row['jenis_status_cucian'] . "</td>";
              echo "</tr>";
            }
            ?>
          </tbody>
        ...
        <table>
          ...
          <tbody>
            ...
            while ($row = mysqli_fetch_assoc($resultTransaksi)) {
              ...
              echo "<td>" . $row['jenis_status_transaksi'] . "</td>";
